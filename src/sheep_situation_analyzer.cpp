 ...
 )
{
  ...
  vector<sheep_transition*> l_real_result;
  while(l_result_iter != l_result_iter_end)
    {
      bool l_valid = true;
      //check validity of transition
      map<pair<unsigned int, unsigned int>,uint32_t>::iterator l_blocked_iter = l_blocked_positions.find((*l_result_iter)->getTarget());
      if(l_blocked_positions.end() != l_blocked_iter)
	{
      ...
      if(l_valid)
	{
          std::cout << "Valid -> " << (*l_result_iter)->toString() << std::endl ;
